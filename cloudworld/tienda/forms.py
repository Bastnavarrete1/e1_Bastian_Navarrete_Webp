...
class RegistroForm(forms.ModelForm):
    ...
    def clean_password(self):
        ...
        if not re.search(r"[A-Z]", password):
            raise forms.ValidationError("Debe tener al menos una mayuscula")

        if not re.search(r"[0-9]", password):
            raise forms.ValidationError("Debe tener al menos un numero")

        if not re.search(r"[!@#$%^&*(),.?\":{}|<>]", password):
            raise forms.ValidationError("Debe tener al menos un simbolo")
